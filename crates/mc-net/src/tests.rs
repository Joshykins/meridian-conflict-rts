...
impl<S: Session> Client<S> {
    ...
    fn pump(&mut self) {
        if let (Some(_), Some(slot), None) =
            (&self.started, self.session.local_player(), &self.ended)
        {
        ...
        for event in self.session.poll() {
            ...
            match event {
                ...
                SessionEvent::SnapshotWanted { tick } => {
                    assert!(
                        self.last_tick().is_none_or(|t| t < tick),
                        "asked for a tick already stepped"
                    );
                ...
                SessionEvent::TickReady(bundle) => {
                    ...
                    let expected = self
                        .last_tick()
                        .map(|t| t + 1)
                        .or(self.loaded_at.map(|t| t + 1))
                        .unwrap_or(0);
                    assert_eq!(
                        bundle.tick, expected,
                        "ticks must arrive in order without gaps"
                    );
                    ...
                    if self.snapshot_at == Some(bundle.tick) {
                        self.session
                            .provide_snapshot(bundle.tick, self.sim.state.to_le_bytes().to_vec())
                            .unwrap();
...
fn pump_until(
    clients: &mut [&mut NetClient],
    what: &str,
    done: impl Fn(&[&mut NetClient]) -> bool,
) {
...
fn start_match(addr: SocketAddr, n: usize) -> Vec<NetClient> {
    ...
    pump_until(&mut refs, "match started", |cs| {
...
fn assert_same_history(a: &FakeSim, b: &FakeSim, at_least: usize) {
    let common: Vec<u32> = a
        .history
        .keys()
        .filter(|t| b.history.contains_key(t))
        .copied()
        .collect();
    assert!(
        common.len() >= at_least,
        "only {} ticks in common",
        common.len()
    );
    for t in common {
...
fn three_clients_receive_identical_bundles() {
    ...
    pump_until(&mut refs(&mut clients), "200 ticks everywhere", |cs| {
        cs.iter().all(|c| c.ticks() >= 200)
    });
    ...
    for (i, c) in clients.iter().enumerate() {
        ...
        assert!(
            executed.len() > 10,
            "slot {i} only got {} commands through",
            executed.len()
        );
        assert_eq!(executed[..], c.sent[..executed.len()]);
    }
    assert!(clients[0].bundles[..start.input_delay as usize]
        .iter()
        .all(|b| b.is_empty()));

    pump_until(&mut refs(&mut clients), "latency measured", |cs| {
        cs.iter().all(|c| c.session.latency().is_some())
    });
    assert!(clients
        .iter()
        .all(|c| c.desync.is_none() && c.ended.is_none()));

    let summary = relay.shutdown().unwrap();
    assert!(summary.ticks >= 200 && summary.desync_tick.is_none());
    pump_until(&mut refs(&mut clients), "relay shutdown seen", |cs| {
        cs.iter().all(|c| c.ended.is_some())
    });
...
fn hash_mismatch_reaches_everyone() {
    ...
    pump_until(&mut refs(&mut clients), "desync reported", |cs| {
    ...
    pump_until(&mut refs(&mut clients), "ticks after the desync", |cs| {
        cs[0].ticks() > seen + 20
    });
...
fn match_continues_when_a_player_drops() {
    let relay = relay(3, |_| {});
    let mut clients = start_match(relay.local_addr(), 3);
    pump_until(&mut refs(&mut clients), "50 ticks", |cs| {
        cs.iter().all(|c| c.ticks() >= 50)
    });

    drop(clients.pop());
    pump_until(&mut refs(&mut clients), "drop noticed", |cs| {
        cs.iter().all(|c| c.dropped == [PlayerId(2)])
    });
    let seen = clients.iter().map(|c| c.ticks()).max().unwrap();
    pump_until(&mut refs(&mut clients), "200 more ticks", |cs| {
        cs.iter().all(|c| c.ticks() >= seen + 200)
    });
...
fn observer_joins_late_from_a_snapshot() {
    ...
    pump_until(&mut refs(&mut clients), "30 ticks", |cs| {
    ...
    pump_until(
        &mut refs(&mut clients),
        "observer caught up and 100 ticks on",
        |cs| {
...
fn player_reconnects_with_token_and_plays_on() {
    ...
    pump_until(
        &mut refs(&mut clients),
        "rejoined and commands executed",
        |cs| {
            let Some(loaded) = cs[1].loaded_at else {
                return false;
            };
            let new_commands = cs[0]
                .bundles
                .iter()
    ...
    let executed: Vec<Vec<u8>> = clients[0]
        ...
        .filter(|(s, c)| {
            *s == PlayerId(1)
                && u32::from_le_bytes(c[1..5].try_into().unwrap()) as usize >= sent_before
        })
        .map(|(_, c)| c.to_vec())
        .collect();
    assert_eq!(
        executed[..],
        back.sent[sent_before..sent_before + executed.len()]
    );
    assert!(clients
        .iter()
        .all(|c| c.desync.is_none() && c.ended.is_none()));
...
fn relay_and_local_replays_play_back_identically() {
    ...
    let mut clients = start_match(relay.local_addr(), 2);
    pump_until(&mut refs(&mut clients), "150 ticks", |cs| {
        cs.iter().all(|c| c.ticks() >= 150)
    });
    ...
    assert!(
        replay.hashes.len() >= 100,
        "agreed hashes are recorded for verification"
    );
    ...
    let mut playback = Client::new(
        ReplaySession::open(&path, Pacing::PerPoll(1000)).unwrap(),
        0,
    );
    while playback.ended.is_none() {
        playback.pump();
...
fn lobby_host_starts_the_match() {
    ...
    pump_until(
        &mut [&mut host, &mut guest, &mut watcher],
        "everyone in the lobby",
        |cs| {
            cs.iter().all(|c| {
                c.lobby
                    .as_ref()
                    .is_some_and(|l| l.players.len() == 2 && l.observers == 1)
            })
        },
    );
    ...
    pump_until(
        ...
        |cs| {
            cs.iter().all(|c| {
                ...
                l.options == [9, 9]
                    && l.players[1].setup == [3, 3]
                    && c.chat == [(Some(PlayerId(1)), "glhf".to_owned())]
            })
        },
    );
    ...
    assert_eq!(
        (lobby.players[0].name.as_str(), lobby.players[0].ready),
        ("host", false)
    );
    ...
    pump_until(
        &mut [&mut host, &mut guest, &mut watcher],
        "guest ready",
        |cs| cs[0].lobby.as_ref().unwrap().players[1].ready,
    );
...
fn handshake_refusals() {
    ...
    pump_until(&mut [&mut host], "lobby intact", |c| {
...
fn read_until<T>(stream: &mut TcpStream, mut want: impl FnMut(Message) -> Option<T>) -> T {
    ...
    loop {
        assert!(
            ...
            "timed out reading from the relay"
        );
...
fn stragglers_are_restamped_and_bad_frames_disconnect() {
    ...
    write_frame(&mut raw, &Message::Ready(true)).unwrap();
    let delay = read_until(&mut raw, |m| {
        if let Message::Start(s) = m {
            Some(s.input_delay)
        } else {
            None
        }
    ...
    let ran_at = read_until(&mut raw, |m| match m {
        Message::Bundle(b)
            if b.commands()
                .any(|(slot, c)| slot == PlayerId(1) && c == b"late") =>
        {
            Some(b.tick)
        }
        _ => None,
    ...
    pump_until(&mut [&mut good], "late command seen", |c| {
        c[0].last_tick() >= Some(ran_at)
    });
    let bundle = &good.bundles[ran_at as usize];
    assert_eq!(
        bundle
            .commands()
            .filter(|(slot, _)| *slot == PlayerId(1))
            .count(),
        1
    );

    // An oversized frame costs the sender its connection and nobody else anything.
    raw.write_all(&((MAX_FRAME_LEN + 1) as u32).to_le_bytes())
        .unwrap();
    ...
    pump_until(&mut [&mut good], "drop noticed", |c| {
        c[0].dropped == [PlayerId(1)]
    });
    let seen = good.ticks();
    pump_until(&mut [&mut good], "match goes on", |c| {
        c[0].ticks() >= seen + 50
    });
