...
fn attachment(format: vk::Format, load: vk::AttachmentLoadOp, final_layout: vk::ImageLayout) -> vk::AttachmentDescription {
...
impl Passes {
    pub fn new(gpu: &Gpu, present_format: vk::Format, present_layout: vk::ImageLayout) -> Result<Passes, GpuError> {
        let color_ref = [vk::AttachmentReference { attachment: 0, layout: vk::ImageLayout::COLOR_ATTACHMENT_OPTIMAL }];
        let make = |attachments: &[vk::AttachmentDescription], color: bool, depth: Option<u32>| -> Result<vk::RenderPass, GpuError> {
            let depth_ref = vk::AttachmentReference { attachment: depth.unwrap_or(0), layout: vk::ImageLayout::DEPTH_STENCIL_ATTACHMENT_OPTIMAL };
            let mut subpass = vk::SubpassDescription::default().pipeline_bind_point(vk::PipelineBindPoint::GRAPHICS);
            if color {
        ...
        let shadow = make(&[attachment(DEPTH_FORMAT, vk::AttachmentLoadOp::CLEAR, vk::ImageLayout::DEPTH_STENCIL_READ_ONLY_OPTIMAL)], false, Some(0))?;
        let scene = make(
            &[
                attachment(HDR_FORMAT, vk::AttachmentLoadOp::CLEAR, vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL),
                attachment(DEPTH_FORMAT, vk::AttachmentLoadOp::CLEAR, vk::ImageLayout::DEPTH_STENCIL_ATTACHMENT_OPTIMAL),
            ],
            true,
            Some(1),
        )?;
        let present = make(&[attachment(present_format, vk::AttachmentLoadOp::DONT_CARE, present_layout)], true, None)?;
        Ok(Passes { shadow, scene, present })
    }
...
fn set_layout(gpu: &Gpu, bindings: &[(u32, vk::DescriptorType)], stages: vk::ShaderStageFlags) -> Result<vk::DescriptorSetLayout, GpuError> {
    let bindings: Vec<_> = bindings
        .iter()
        .map(|&(binding, ty)| vk::DescriptorSetLayoutBinding::default().binding(binding).descriptor_type(ty).descriptor_count(1).stage_flags(stages))
        .collect();
    Ok(unsafe { gpu.device.create_descriptor_set_layout(&vk::DescriptorSetLayoutCreateInfo::default().bindings(&bindings), None) }?)
}
...
impl Layouts {
    pub fn new(gpu: &Gpu) -> Result<Layouts, GpuError> {
        ...
        let screen_set = set_layout(gpu, &[(0, T::SAMPLED_IMAGE), (1, T::SAMPLED_IMAGE), (2, T::SAMPLER)], gfx)?;
        let cull_bindings: Vec<_> = (0..10).map(|i| (i, if i == 0 { T::UNIFORM_BUFFER } else { T::STORAGE_BUFFER })).collect();
        let cull_set = set_layout(gpu, &cull_bindings, vk::ShaderStageFlags::COMPUTE)?;

        let pipeline_layout = |sets: &[vk::DescriptorSetLayout], stages: vk::ShaderStageFlags| -> Result<vk::PipelineLayout, GpuError> {
            let push = [vk::PushConstantRange { stage_flags: stages, offset: 0, size: 8 }];
            let info = vk::PipelineLayoutCreateInfo::default().set_layouts(sets).push_constant_ranges(&push);
    ...
    pub fn destroy(&self, gpu: &Gpu) {
        unsafe {
            ...
            for s in [self.scene_set, self.pass_set, self.screen_set, self.cull_set] {
...
fn graphics_pipeline(gpu: &Gpu, d: &PipelineDesc) -> Result<vk::Pipeline, GpuError> {
    let mut stages = vec![vk::PipelineShaderStageCreateInfo::default().stage(vk::ShaderStageFlags::VERTEX).module(d.module).name(d.vs)];
    stages.push(vk::PipelineShaderStageCreateInfo::default().stage(vk::ShaderStageFlags::FRAGMENT).module(d.module).name(d.fs));

    let f = |location, format, offset| vk::VertexInputAttributeDescription { location, binding: 0, format, offset };
    let (stride, attributes): (u32, Vec<vk::VertexInputAttributeDescription>) = match d.vertex {
        ...
        VertexKind::Overlay => (32, vec![f(0, vk::Format::R32G32_SFLOAT, 0), f(1, vk::Format::R32G32_SFLOAT, 8), f(2, vk::Format::R32G32B32A32_SFLOAT, 16)]),
    };
    let bindings = [vk::VertexInputBindingDescription { binding: 0, stride, input_rate: vk::VertexInputRate::VERTEX }];
    let mut vertex_input = vk::PipelineVertexInputStateCreateInfo::default();
    if d.vertex != VertexKind::None {
        vertex_input = vertex_input.vertex_binding_descriptions(&bindings).vertex_attribute_descriptions(&attributes);
    }
    ...
    let depth = vk::PipelineDepthStencilStateCreateInfo::default()
        ...
        .depth_compare_op(if d.depth == Depth::Shadow { vk::CompareOp::LESS_OR_EQUAL } else { vk::CompareOp::GREATER_OR_EQUAL });
    ...
    let info = [vk::GraphicsPipelineCreateInfo::default()
        ...
        .layout(d.layout)
        .render_pass(d.pass)];
    unsafe { gpu.device.create_graphics_pipelines(vk::PipelineCache::null(), &info, None) }.map(|p| p[0]).map_err(|(_, e)| GpuError::Vk(e))
}

fn compute_pipeline(gpu: &Gpu, module: vk::ShaderModule, entry: &CStr, layout: vk::PipelineLayout) -> Result<vk::Pipeline, GpuError> {
    ...
    unsafe { gpu.device.create_compute_pipelines(vk::PipelineCache::null(), &info, None) }.map(|p| p[0]).map_err(|(_, e)| GpuError::Vk(e))
}
...
impl Pipelines {
    pub fn new(gpu: &Gpu, layouts: &Layouts, passes: &Passes) -> Result<Pipelines, GpuError> {
        ...
        let scene = |module, vs, fs, vertex, blend, depth, cull| graphics_pipeline(gpu, &PipelineDesc { module, vs, fs, layout: layouts.scene, pass: passes.scene, vertex, blend, depth, cull });
        let shadow = |module, vs, fs, vertex| {
            graphics_pipeline(gpu, &PipelineDesc { module, vs, fs, layout: layouts.scene, pass: passes.shadow, vertex, blend: Blend::NoColor, depth: Depth::Shadow, cull: none })
        };
        let present = |vs, fs, vertex, blend| {
            graphics_pipeline(gpu, &PipelineDesc { module: screen, vs, fs, layout: layouts.screen, pass: passes.present, vertex, blend, depth: Depth::Off, cull: none })
        };

        Ok(Pipelines {
            terrain: scene(terrain, c"vs_main", c"fs_main", VertexKind::Vec2, Blend::Opaque, Depth::TestWrite, back)?,
            terrain_shadow: shadow(terrain, c"vs_main", c"fs_shadow", VertexKind::Vec2)?,
            entity: scene(entity, c"vs_main", c"fs_main", VertexKind::Mesh, Blend::Opaque, Depth::TestWrite, back)?,
            entity_shadow: shadow(entity, c"vs_main", c"fs_shadow", VertexKind::Mesh)?,
            stain: scene(ground, c"vs_stain", c"fs_stain", VertexKind::Vec2, Blend::Alpha, Depth::Test, none)?,
            water: scene(ground, c"vs_water", c"fs_water", VertexKind::None, Blend::Alpha, Depth::Test, none)?,
            icon: scene(icons, c"vs_icon", c"fs_icon", VertexKind::Vec2, Blend::Alpha, Depth::Off, none)?,
            ring: scene(icons, c"vs_ring", c"fs_ring", VertexKind::Vec2, Blend::Alpha, Depth::Test, none)?,
            bar: scene(icons, c"vs_bar", c"fs_bar", VertexKind::Vec2, Blend::Alpha, Depth::Off, none)?,
            projectile: scene(sprites, c"vs_projectile", c"fs_sprite", VertexKind::Vec2, Blend::Additive, Depth::Test, none)?,
            effect: scene(sprites, c"vs_effect", c"fs_sprite", VertexKind::Vec2, Blend::Additive, Depth::Test, none)?,
            tonemap: present(c"vs_fullscreen", c"fs_tonemap", VertexKind::None, Blend::Opaque)?,
            overlay: present(c"vs_overlay", c"fs_overlay", VertexKind::Overlay, Blend::Alpha)?,
            ...
            modules: vec![terrain, entity, ground, icons, sprites, screen, cull],
        })
    }
    ...
    pub fn destroy(&self, gpu: &Gpu) {
        unsafe {
            for p in [
                self.terrain, self.terrain_shadow, self.entity, self.entity_shadow, self.stain, self.water, self.icon, self.ring, self.bar,
                self.projectile, self.effect, self.tonemap, self.overlay, self.cull_clear, self.cull_cull, self.cull_prefix, self.cull_scatter,
            ] {
