...
fn map_list(ui: &mut Ui, state: &mut SkirmishState, area: Rect) {
    ...
    for (i, entry) in state.maps.iter().enumerate().take(6) {
        ...
        ui.text(
            ...
            rgb(palette::DIM, 1.0),
            &format!(
                "{km:.0} KM  \u{b7}  {} PLAYERS  \u{b7}  {} DEPOSITS",
                entry.map.start_positions().len().min(8),
                entry.map.mass_deposits().len()
            ),
        );
    }
    if let Some(i) = pick {
...
fn rules(ui: &mut Ui, state: &mut SkirmishState, area: Rect) {
    ...
    ui.text_field(
        id("rule-name", 0),
        Rect::new(r.right() - 220.0, r.y + 8.0, 220.0, 34.0),
        &mut state.name,
        16,
    );
}
...
fn map_preview(ui: &mut Ui, state: &mut SkirmishState, area: Rect) {
    ...
    ui.text_right(
        ...
        type_scale::MICRO,
        rgb(palette::DIM, 1.0),
        "GREEN: MASS DEPOSITS    RINGS: LANDING ZONES",
    );
}
...
fn commanders(ui: &mut Ui, state: &mut SkirmishState, area: Rect) {
    ...
    for i in 0..starts {
        ...
        let res = ui.interact(
            id("slot-color", i),
            Rect::new(row.x, row.y, 46.0, row.h),
            open,
        );
        ui.fill(
            Rect::new(chip.x, chip.y, chip.w + 4.0 * res.glow, chip.h),
            [c[0], c[1], c[2], live],
        );
        if res.clicked {
    ...
    if y + 60.0 < area.bottom() {
        ...
        for (k, line) in [
            "Destroy every enemy commander to win.",
            "Commanders and engineers are amphibious: islands are",
            "reached on foot along the sea bed, or by hover tank.",
        ]
        .into_iter()
        .enumerate()
        {
            ui.text(
                area.x + 12.0,
                y + 28.0 + k as f32 * 22.0,
                super::style(mc_render::Face::Medium, 14.5, 1.2),
                rgb(palette::DIM, 1.0),
                line,
            );
        }
