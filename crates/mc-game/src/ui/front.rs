...
pub struct Front {
    ...
    launching: Option<(MatchRequest, f32)>,
    quitting: Option<f32>,
}
...
impl Front {
    pub fn new(director: Director) -> Front {
        Front { screen: Screen::Menu, target: Screen::Menu, enter: 0.0, menu: MenuState::default(), skirmish: None, director, launching: None, quitting: None }
    }
    ...
    fn go(&mut self, screen: Screen, settings: &Settings) {
        if screen == Screen::Skirmish && self.skirmish.is_none() {
            self.skirmish = Some(SkirmishState::new(&settings.skirmish_map, settings.skirmish_fog, &settings.player_name));
        }
        self.target = screen;
    }

    pub fn frame(&mut self, ui: &mut Ui, settings: &mut Settings, telemetry: &Telemetry) -> FrontOutcome {
        let mut out = FrontOutcome::default();
        self.director.update(ui.dt);
        ui.fill(Rect::new(0.0, 0.0, ui.size.x, ui.size.y), rgb(0x000000, self.director.dip()));

        // Leave, swap, arrive.
        let leaving = self.target != self.screen || self.launching.is_some() || self.quitting.is_some();
        if self.target != self.screen {
            self.enter -= ui.dt / LEAVE;
            if self.enter <= 0.0 {
                self.enter = 0.0;
                self.screen = self.target;
                ui.audio.play(Sfx::Whoosh);
            }
        } else if !leaving {
        ...
        match self.screen {
            Screen::Menu => match menu::draw(ui, &mut self.menu, &mut self.director, telemetry, enter) {
            ...
            Screen::Skirmish => {
                ...
                match skirmish::draw(ui, state, enter) {
                    ...
                    Some(SkirmishAction::Start(request)) => self.launching = Some((request, 0.0)),
                    None => {}
                }
                // What was set up is what the screen opens with next time.
                let name = crate::settings::clean_name(&state.name);
                if settings.skirmish_map != state.selected_stem() || settings.skirmish_fog != state.fog || (settings.player_name != name && ui.mem.editing.is_none()) {
        ...
        if let Some((_, t)) = &mut self.launching {
            *t += ui.dt;
            ...
            ui.fill(Rect::new(0.0, 0.0, ui.size.x, ui.size.y), rgb(0x000000, k * k));
            if *t >= LAUNCH_FADE + 0.05 {
                out.event = self.launching.take().map(|(request, _)| FrontEvent::Launch(request));
            }
        }
        if let Some(t) = &mut self.quitting {
            *t += ui.dt;
            ui.fill(Rect::new(0.0, 0.0, ui.size.x, ui.size.y), rgb(0x000000, (*t / 0.3).min(1.0)));
            if *t >= 0.32 {
...
pub fn loading_card(ui: &mut Ui, title: &str, detail: &str) {
    ...
    for (radius, speed, sweep) in [(40.0, 1.6, 1.2), (54.0, -1.1, 2.0), (70.0, 0.7, 0.8)] {
        ui.arc(c, radius, t * speed, t * speed + sweep, 1.4, rgb(palette::ACCENT, 0.7));
    }
    ui.text_centred(w * 0.5 + 7.0, c.y + 120.0, super::type_scale::OVERLINE, rgb(palette::TEXT, 1.0), title);
    ui.text_centred(w * 0.5, c.y + 150.0, super::type_scale::MICRO, rgb(palette::DIM, 1.0), detail);
}
