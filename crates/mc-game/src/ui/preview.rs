...
fn ramp(stops: &[(f32, [f32; 3])], v: f32) -> [f32; 3] {
    let i = stops.iter().rposition(|(at, _)| v >= *at).unwrap_or(0).min(stops.len() - 2);
...
pub fn render(map: &MapFile) -> Vec<u8> {
    ...
    let height = |x: f32, y: f32| {
        ...
        let (top, bottom) = (z(x0, y0) * (1.0 - fx) + z(x0 + 1, y0) * fx, z(x0, y0 + 1) * (1.0 - fx) + z(x0 + 1, y0 + 1) * fx);
    ...
    let (w, h) = ((size_m[0] / metres_per_px) as usize, (size_m[1] / metres_per_px) as usize);
    let (pad_x, pad_y) = ((SIZE - w) / 2, (SIZE - h) / 2);

    let land = [(0.0, [0.30, 0.34, 0.27]), (12.0, [0.20, 0.27, 0.22]), (45.0, [0.25, 0.30, 0.24]), (90.0, [0.36, 0.37, 0.30]), (160.0, [0.47, 0.46, 0.41]), (300.0, [0.72, 0.74, 0.76])];
    let sea = [(0.0, [0.10, 0.27, 0.33]), (6.0, [0.06, 0.17, 0.23]), (30.0, [0.035, 0.10, 0.15]), (70.0, [0.02, 0.06, 0.10])];

    let mut rgba = vec![0u8; SIZE * SIZE * 4];
    for py in 0..h {
        for px in 0..w {
            // Image rows run top to bottom; the map's +Y is north.
            let (x, y) = ((px as f32 + 0.5) * step, (h as f32 - py as f32 - 0.5) * step);
            let z = height(x, y);
            ...
            let mut c = if z > 0.0 {
                ...
                let f = (z / 25.0).fract();
                let line = (1.0 - f.min(1.0 - f) * 25.0 / (slope * metres_per_px) / 0.8).clamp(0.0, 1.0);
                ramp(&land, z).map(|v| v * shade * (1.0 - 0.22 * line))
            } else {
                ramp(&sea, -z)
            };
            // The shoreline glows: a band either side of the water's edge.
            let shore = (1.0 - (z.abs() / (2.5 + (gx * gx + gy * gy).sqrt() * metres_per_px * 0.7))).clamp(0.0, 1.0);
    ...
    // Mass deposits as small amber marks.
    for d in map.mass_deposits() {
        let p = d.to_f32();
        let (cx, cy) = ((p[0] / metres_per_px) as i64 + pad_x as i64, (h as f32 - p[1] / metres_per_px) as i64 + pad_y as i64);
        for (dx, dy) in [(0, 0), (1, 0), (-1, 0), (0, 1), (0, -1)] {
            let (x, y) = (cx + dx, cy + dy);
            if (0..SIZE as i64).contains(&x) && (0..SIZE as i64).contains(&y) {
                let at = (y as usize * SIZE + x as usize) * 4;
                rgba[at..at + 4].copy_from_slice(&[244, 190, 92, 255]);
            }
        }
    }
    rgba
